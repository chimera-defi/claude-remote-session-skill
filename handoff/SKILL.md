...
2. **Verify healthy.** `session-handoff check <tmux-session>` — exit 0 means ready at the prompt. Don't send to a `starting`/`dead`/`menu` target (`menu` covers both interactive-widget panes and Claude Code's first-launch folder-trust dialog — `send` refuses these outright, it does not queue); `busy` will queue behind current work. Full state list + why: `_state_of`'s comment in `scripts/session-handoff.sh`, not restated here.
