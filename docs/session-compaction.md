...
5. **One `/compact` invocation writes FIVE `type:user` artifacts, not one**, so a
   ...
   re-compacts it ~30min later, forever (happened for real, 2026-09-11). All five
...
**The unit `install-timer` generates runs `report` mode.** (As of 2026-09-24 it has
never been enabled on this host — `systemctl --user is-enabled
session-compact-report.timer` → `not-found`. The compact timer that *is* live is the
managed-only one described under "Deliberately not shipped".) Enabling it is safe: it logs who *would* be
...
- An enabled timer that sends `/compact` unattended to the **whole fleet** (reasons
  above). The one exception is live: a hand-deployed `session-autocompact.timer`
  (not generated by `install-timer`) has run `session-compact sweep --apply
  --managed-only` every 30 min since 2026-09-12, scoped to the allowlisted managed
  orchestrators and governed by the two 2026-09-22 policies below. Check it with
  `systemctl --user status session-autocompact.timer` and
  `journalctl --user -u session-autocompact.service`.
