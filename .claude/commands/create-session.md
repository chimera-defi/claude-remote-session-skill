...
2. Run the `new-session` script for the whole recipe in one Bash call:
   ...
   ```
   If `~/.local/bin/new-session` is missing, install it from `scripts/new-session.sh` (copy to `~/.local/bin/new-session` and `chmod +x`) and re-run the command above — do not hand-roll the start script/systemd unit inline; the script already handles aliasing (`session-alias`), git-aware run-dir resolution (`session-git-prep`), sentinel-file/backoff logic, and the first-party `ANTHROPIC_BASE_URL` settings override. Only if the repo itself is unavailable, fall back to the one-off manual recipe in `references/fallback-recipe.md` (paste its whole block in a single Bash call) — it's a deliberately reduced last resort for when nothing else is reachable, not a way to recreate the installed script. It drops: store lookup only (no acronym inference for un-stored long folders); and no same-minute collision lock (a second same-minute spawn of the same folder can collide/no-op instead of getting a disambiguated name). The kickoff-verification retry (wait for the pane's shell, then retry the launch `Enter` with verification) IS ported into the fallback recipe and kept in sync by `tests/test-fallback-recipe-sync.sh`.
